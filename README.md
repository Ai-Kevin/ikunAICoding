...
│  Axios · ECharts         │         Bearer Token         │   SQLite                   │
└─────────────────────────┘                              └──────────────────────────┘
```
...
> **后端特性：** RESTful API · JWT 鉴权 · CORS 跨域 · 自动 Swagger 文档 · 启动自动建表与种子数据。
...
| [Pinia](https://pinia.vuejs.org/) | 2.3 | 状态管理（用户 / Token） |
...
> **前端特性：** 前后端分离 · JWT 自动注入 · 路由鉴权 · 响应式布局 · ECharts 看板 · Vite 代理转发。
...
| 模块 | 说明 |
| --- | --- |
| **登录页** | 账号密码登录、记住我、第三方登录入口（企业微信 / GitLab / LDAP） |
...
│   ├── src/
│   │   ├── api/              # Axios 封装与接口
│   │   ├── components/       # 公共组件（KvEditor / CodeEditor / ECharts 封装）
│   │   ├── layouts/          # 主框架布局（侧边栏 + 顶栏）
│   │   ├── router/           # 路由与守卫
│   │   ├── store/            # Pinia 状态
│   │   ├── styles/           # 全局样式与设计变量
│   │   └── views/            # 页面（登录 / 看板 / 项目 / API / UI / 计划）
│   ├── vite.config.js
│   └── package.json
├── docs/                     # 文档与截图资源
└── README.md
